...
const AnimatedBento: React.FC = () => {
    return (
        <section className="py-12 px-4 md:px-6 max-w-6xl mx-auto bg-white">
            {/* Header */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-10 gap-4 px-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4 h-auto md:h-[550px]">
                ...
                <div className="md:col-span-2 bg-slate-100 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 relative overflow-hidden group cursor-pointer shadow-lg border border-slate-200 min-h-[180px]">
                    ...
                    <div className="relative z-10 flex justify-between items-start">
                        <div>
                            <h2 className="text-2xl font-black text-slate-900 italic tracking-tighter">AC Revive</h2>
                            <p className="text-indigo-600 font-bold text-[10px] mt-1 uppercase tracking-widest">Upto 30% OFF</p>
                        </div>
...
const ProcessFlow: React.FC = () => {
    ...
    return (
        <section className="py-16 md:py-24 px-4 md:px-6 bg-white rounded-[2.5rem] md:rounded-[4rem] my-10 max-w-7xl mx-auto shadow-sm border border-slate-50">
            <div className="text-center mb-16 md:mb-20">
                <h2 className="text-3xl md:text-4xl font-black text-slate-900 italic leading-none px-4">Simple 3-Step Booking</h2>
                <div className="h-1.5 w-20 md:w-24 bg-indigo-600 mx-auto mt-6 rounded-full"></div>
            </div>

            <div
                ref={containerRef}
                className="relative flex flex-col md:flex-row justify-between items-center md:items-start gap-16 md:gap-12"
            >
                {steps.map((item, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center text-center relative z-10 group w-full max-w-[280px]">
                        <div
                            ref={stepRefs[i]}
                        ...
                        <p className="text-slate-400 font-bold text-[10px] md:text-xs uppercase tracking-widest leading-relaxed">{item.desc}</p>
                    </div>
                ))}

                {/* Beams - Visible only on Desktop */}
