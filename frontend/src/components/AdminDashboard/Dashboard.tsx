            ...
            `}>
                ...
                <div className="flex items-center gap-4 mb-10 px-2 shrink-0">
                    ...
                    <span className="font-black text-2xl tracking-tighter text-slate-900">
                        Handyman <br /><span className="text-indigo-600 text-[10px] uppercase tracking-[0.3em] font-bold">Admin Panel</span>
                    </span>
                </div>

                {/* Navigation - flex-1 pushes footer down */}
                <nav className="flex flex-col gap-2 flex-1 overflow-y-auto no-scrollbar pr-1">
                    <SidebarItem id="dashboard" label="Dashboard" icon={<LayoutDashboard size={18} />} activeTab={activeTab} setActiveTab={setActiveTab} onItemClick={() => setIsMobileMenuOpen(false)} />
                    <SidebarItem id="services" label="Services" icon={<Package size={18} />} activeTab={activeTab} setActiveTab={setActiveTab} onItemClick={() => setIsMobileMenuOpen(false)} />
                ...
                {/* Sidebar Footer - Always at the very bottom */}
            ...
            <main className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
                ...
                <header className="px-6 md:px-10 py-5 flex justify-between items-center bg-[#F4F7FE]/80 backdrop-blur-xl sticky top-0 z-40 border-b border-slate-200/50">
                    <div className="flex items-center gap-4">
                        <button 
                            onClick={toggleMobileMenu}
                            className="lg:hidden p-2.5 bg-white rounded-xl shadow-sm border border-slate-200 text-slate-600 hover:bg-slate-50 transition-colors"
                        >
                            <Menu size={20} />
                        </button>
                        <div>
